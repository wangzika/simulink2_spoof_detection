...
The platform converts GPS time and Unix time onto a common axis, extracts per-satellite code, carrier, signal-strength, and loss-of-lock features, computes GPS broadcast-ephemeris pseudorange residuals, injects controlled observation-level spoofing windows, and reports precision, recall, F1 score, ROC AUC, false-alarm rate, and detection latency.
...
The first transparent baseline achieves recall \AttackRecall{} and ROC AUC \AttackRocAuc{} under synthetic spoofing, while the raw residual layer provides \RawRaimCoverageRows{} synchronized RAIM-capable epochs for baseline and ablation studies.
\end{abstract}
...
\begin{itemize}
    ...
    \item a GPS broadcast-ephemeris residual engine with weighted least-squares positioning, RAIM chi-square scoring, and reference-position pseudorange residuals;
    ...
    \item an observation-level pseudorange attack injector and a first transparent baseline with reproducible metrics and figures.
\end{itemize}
...
For GPS observations, the platform also parses RINEX broadcast navigation records and computes satellite ECEF positions, satellite clock corrections, elevation/azimuth, pseudorange residuals, and weighted RAIM statistics.
The RAIM baseline solves a GPS-only weighted least-squares position and receiver clock bias at each epoch, then evaluates the post-fit residual chi-square statistic.
In parallel, a reference residual is computed with the RTKLIB ECEF trajectory to expose cross-source pseudorange consistency; its mean RMS over the synchronized run is \RawReferenceResidualMean{} m.
...
\begin{equation}
\begin{split}
S ={}& w_r S_r + w_m S_m + w_p S_p + w_a S_a \\
&+ w_\rho S_\rho + w_c S_c + w_d S_d + w_q S_q ,
\end{split}
\end{equation}
where $S_r$ is the LiDAR--GNSS residual score, $S_m$ is the Mahalanobis gate score, $S_p$ and $S_a$ are receiver pseudorange RMS and maximum-residual scores, $S_\rho$ is the raw RAIM score, $S_c$ is the reference pseudorange residual score, $S_d$ is the Doppler/TDCP score when available, and $S_q$ penalizes degraded RTK quality.
...
Fig.~\ref{fig:raim} shows the broadcast-ephemeris raw residual layer under the observation-level attack injector.
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=\linewidth]{raw_raim_timeline.png}
    \caption{Raw GPS pseudorange residual timeline. The observation-level injector modifies satellite pseudoranges before the broadcast-ephemeris residual and RAIM statistics are recomputed.}
    \label{fig:raim}
\end{figure}
...
The next research stage should move from the current GPS-only raw residual baseline to multi-constellation residual modeling.
Specifically, we will add Galileo and BeiDou broadcast ephemerides, ionosphere/troposphere correction options, Doppler/TDCP residuals, and RAIM-only, LiDAR--GNSS-only, raw-GNSS-only, and fused adaptive baselines.
