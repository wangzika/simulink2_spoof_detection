...
static bool sync_publish_to_clock = false;
static double clock_wait_timeout = 30.0;
static double publish_time_tolerance = 0.002;

static bool wait_for_publish_time(const ros::Time& stamp)
{
    if (!sync_publish_to_clock) return ros::ok();

    ros::WallTime start = ros::WallTime::now();
    ros::WallDuration sleep_step(0.001);

    while (ros::ok() && ros::Time::now().isZero())
    {
        if (clock_wait_timeout > 0.0 &&
            (ros::WallTime::now() - start).toSec() > clock_wait_timeout)
        {
            ROS_WARN_THROTTLE(5.0, "waiting for /clock timed out before GNSS publication");
            return false;
        }
        ros::spinOnce();
        sleep_step.sleep();
    ...
    while (ros::ok() &&
           (stamp - ros::Time::now()).toSec() > publish_time_tolerance)
    {
        if (clock_wait_timeout > 0.0 &&
            (ros::WallTime::now() - start).toSec() > clock_wait_timeout)
        {
            ROS_WARN_THROTTLE(5.0, "waiting for GNSS stamp %.3f timed out at ROS time %.3f",
...
extern void rtkposRegisterPub(ros::NodeHandle& n)
{
    ...
    ros::NodeHandle pnh("~");
    pnh.param("sync_to_clock", sync_publish_to_clock, false);
    pnh.param("clock_wait_timeout", clock_wait_timeout, 30.0);
    pnh.param("publish_time_tolerance", publish_time_tolerance, 0.002);
    ...
    const nav_t* nav)
{
...

    {
        epoch.stat = stat;
        epoch.ns = rtk->sol.ns;
        epoch.age = rtk->sol.age;
        epoch.ratio = rtk->sol.ratio;
        epoch.thres = rtk->sol.thres;
        for (i = 0; i < 4; i++)
        {
            epoch.dop[i] = rtk->sol.dop[i];
        }
        ...
        if (stat != SOLQ_NONE)
        {
            ...
            odometry.pose.covariance[0] = epoch.var[0];
            odometry.pose.covariance[7] = epoch.var[1];
            odometry.pose.covariance[14] = epoch.var[2];
            odometry.pose.covariance[1] = odometry.pose.covariance[6] = epoch.var[3];
            odometry.pose.covariance[8] = odometry.pose.covariance[13] = epoch.var[4];
            odometry.pose.covariance[2] = odometry.pose.covariance[12] = epoch.var[5];
            odometry.pose.covariance[21] = 9999.0;
            odometry.pose.covariance[28] = 9999.0;
            odometry.pose.covariance[35] = 9999.0;
            ...
            // if (posvar < 3)
            {
                if (wait_for_publish_time(epoch.header.stamp))
                {
