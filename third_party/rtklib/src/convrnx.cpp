...
typedef struct halfc_tag
{      /* half-cycle ambiguity list type */
    gtime_t ts, te;              /* first and last observation time */
...
static void convcode(int rnxver, int sys, char* type)
{
    ...
    else if (rnxver >= 212 && sys == SYS_GLO && !strcmp(type + 1, "1C"))
    { /* L1C/A */
        strcpy(type + 1, "A");
    }
    else if (rnxver >= 212 && sys == SYS_GLO && !strcmp(type + 1, "2C"))
    { /* L2C/A */
        strcpy(type + 1, "D");
    }
    ...
    { /* L1P,P(Y) */
        ...
        !strcmp(type, "C2N") || !strcmp(type, "C2D"))
    { /* L2P,P(Y) */
        strcpy(type, "P2");
    }
    else
