    ...
    const solopt_t* opt)
{
    ...
    //    p+=sprintf(p,"%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s"
    //                 "%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f",
    //                 s,sep,sol->rr[3],sep,sol->rr[4],sep,sol->rr[5],sep,sol->stat,sep,
    //                 sol->ns,sep,SQRT(sol->qr[0]),sep,SQRT(sol->qr[1]),sep,
    //                 SQRT(sol->qr[2]),sep,sqvar(sol->qr[3]),sep,sqvar(sol->qr[4]),sep,
    //                 sqvar(sol->qr[5]),sep,sol->age,sep,sol->ratio);
    ...
    const double* rb, const solopt_t* opt)
{
    ...
    trace(3, "outenu  :\n");
    //    for (i=0;i<3;i++) rr[i]=sol->vv[i];
    //    for (i=0;i<3;i++) rr[i]=sol->rr[i]-rb[i];
    //    for (i=0;i<3;i++) rr[i]=sol->dopvel[i];
    //    for (i=0;i<3;i++) rr[i]=sol->tdcpvel[i];
    for (i = 0;i < 3;i++) rr[i] = sol->checkvel[i];
    //    for (i=0;i<3;i++) rr[i]=sol->sddopvel[i];
    //    for (i=0;i<3;i++) rr[i]=sol->predict_vel[i]-rb[i];
    ecef2pos(rb, pos);
    soltocov(sol, P);
    covenu(pos, P, Q);
    ecef2enu(pos, rr, enu);
    p += sprintf(p, "%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s"
        "%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\r\n",
        s, sep, enu[0], sep, enu[1], sep, enu[2], sep, sol->checkstat, sep, sol->ns, sep,
